...
plot(all.worldclim[["bio1"]]) #temperature is coded as bio1
plot(all.worldclim[["bio12"]]) #precipitation is coded as bio12 
plot(all.worldalt[["alt"]]) #altitude is coded as alt 
```
...
The peninsula mapped above is SPAIN 
2. Is altitude a climate variable? How do the peninsula maps plotted above support your answer?
The altitude seems to be variable but the climate seems to be constant given by the complete green coloration of the area. 
...
given by the code done in class below. 1
...
#for porfolio part 4.3 
results=gnr_resolve(names="Castanea dentata")
head(results$matched_name)
...
results=gnr_resolve(names="Acer")
head(results$matched_name)
name="Acer"  
tsn=get_tsn(name, accepted=FALSE)
tsn 
...
#this is where you should add a 1 so that the code ends 
```
...
1. Spergularia melanocaulos
2. Petrocoptis grandiflora 

WORK WITH TAXISE
1. Look up - are they accepted names or are they synonyms?
Spergularia melanocaulos Merino is a synonym for Spergularia melanocaulos 
...
head(results$matched_name)
results=gnr_resolve(names="Petrocoptis grandiflora")
head(results$matched_name)

```

2. Look up the two plant taxa. Are they both angiosperms?
According to the code it traced bot to NA. I am not sure what this information means. 
```{r}
...
I am not sure if there is no data for the taxa or if I am doing something wrong but I keep getting NA for family for each species. 
```{r}
...
There was no species given to me by the data frame 


5. For one species, look up to see if there are subspecies or other infraspecific forms.
I was unable to code for this because there were no subspecies shown 
...
```{r}
Grandifloras <- occ(q="Petrocoptis grandifloras", from="gbif", limit=50)
Grandifloras

Grandifloras.dat <- (Grandifloras$gbif$data)
head(Grandifloras.dat)

Grandifloras.df <- occ2df(Grandifloras)
Grandifloras.df

dim(Grandifloras.df)
Grandifloras.df <- Grandifloras.df[complete.cases(Grandifloras.df ),]
dim(Grandifloras.df)
Grandifloras.df


#now working with the next data Spergularia melanocaulos 
DawnRedwood2 <- occ(q="Spergularia melanocaulos", from="gbif", limit=50)
DawnRedwood2

DawnRedwood2.dat <- (DawnRedwood2$gbif$data)
head(DawnRedwood2.dat)

DawnRedwood.df2 <- occ2df(DawnRedwood2)
DawnRedwood.df2

dim(DawnRedwood.df2)
DawnRedwood.df2 <- DawnRedwood.df2[complete.cases(DawnRedwood.df2),]
dim(DawnRedwood.df2)
DawnRedwood.df2
...
here I decided to change to use another species because for the species I was working with I could not get significant data. 
```{r}
P_hispanica <- BIEN_occurrence_species(species = "Petrocoptis hispanica") 
P_hispanica_rangemap <- BIEN_ranges_load_species("Petrocoptis hispanica") 
...
plot(all.worldclim[["bio1"]])
points(P_hispanica[,c("longitude", "latitude")], pch = 24, cex = 0.5) 
```
...
somwhow there is an error that occurs in the lists where its telling me its trying to get the polygons but I dont know what it is reffering to. 
```{r}
M_recurva <- BIEN_occurrence_species(species = "Minuartia recurva") 
M_recurva_rangemap <- BIEN_ranges_load_species("Minuartia recurva")  
